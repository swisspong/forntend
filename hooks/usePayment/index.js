...
import { createOrderCODAuthReq } from "../../lib/axiosPrivate";
...
export const useAddOrderCODNoAuthMutation = () => {
  const queryClient = useQueryClient();
  return useMutation(
    (order) => API.post(`/order/${Cookies.get("cart_id")}`, order),
    {
...
export const useAddOrderCODAuthMutation = () => {
  const queryClient = useQueryClient();
  const credential = useSelector(selectCredential);
  return useMutation(
    (order) => createOrderCODAuthReq(credential.user.id, order),
    {
      // When mutate is called:
      onMutate: async (order) => {
        // Cancel any outgoing refetches (so they don't overwrite our optimistic update)
